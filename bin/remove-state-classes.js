...
function filterStateClasses(classString) {
	return classString
		.split(/\s+/)
		.filter((token) => token && !isStateClass(token))
		.join(" ")
		.trim();
}
...
function processContent(content) {
	...
	result = result.replace(/class:\s*"([^"]*)"/g, (_match, classes) => {
		const filtered = filterStateClasses(classes);
		return `class: "${filtered}"`;
	});
	...
	result = result.replace(/"([^"\n]+)"/g, (match, content) => {
		...
		const hasStateClass =
			STATE_PREFIXES.some((prefix) => content.includes(prefix)) ||
			content.includes("[&:hover]") ||
			content.includes("[&:focus]") ||
			content.includes("[&[data-active]]") ||
...
async function main() {
	...
	console.log(`\n${"─".repeat(50)}`);
	console.log(
		`📊 Summary: ${changedCount} of ${files.length} files ${DRY_RUN ? "would be" : "were"} modified`,
	);
