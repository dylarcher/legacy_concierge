...
/**
 * Mobile navigation panel component with accordion dropdowns.
 *
 * @element mobile-nav
 * @attr {string} brand - Brand name for accessibility
 *
 * @fires menu-open - When mobile menu opens
 * @fires menu-close - When mobile menu closes
 *
 * @example
 * <mobile-nav brand="Legacy Concierge"></mobile-nav>
 *
 * // Open programmatically:
 * document.querySelector('mobile-nav').open();
 */
class MobileNav extends BaseComponent {
	...
	/**
	 * Injects component-specific styles
	 * @returns {void}
	 */
	#injectComponentStyles() {
		...
		style.textContent = `
			...
			mobile-nav .mobile-menu-backdrop.hidden {
				opacity: 0;
				pointer-events: none;
			}
			mobile-nav .mobile-menu-panel.hidden {
	...
	/**
	 * Updates visibility and manages focus trap
	 * @returns {void}
	 */
	#updateVisibility() {
	...
	/**
	 * Creates the close button icon SVG
	...
	render() {
		const brand = this.getAttribute("brand") || "Legacy Concierge";
		...
		const treatmentLinks = [
			...
			{ href: resolvePath("/pages/treatments/rehab"), text: "Rehabilitation" },
		];
			...
			this.h(
				...
				this.h(
					...
					this.h(
						...
						this.#createMobileDropdown("Home", homeLinks),
						this.#createMobileDropdown("Treatments", treatmentLinks),
						this.#createMobileDropdown("Expertise", expertiseLinks),
						this.h(
							"a",
							{
...
defineElement("mobile-nav", MobileNav);
